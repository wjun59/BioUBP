...
def sc_psednc_general(rna_sequences, pyche_df, pyche_list, lambda_value, weight):
    """
    计算多个RNA序列的SC-PseDNC-General特征
    ...
    返回：
    list，每个RNA序列的SC-PseDNC-General特征向量
    """
    all_feature_vectors = []
            ...
            for i in range(L - j - 1):
                ...
                if dinucleotide_1 in pyche_df.columns and dinucleotide_2 in pyche_df.columns:
                    ...
                    for idx in pyche_list:
                        ...
                        sum_property_product += P1 * P2
                    theta_j += sum_property_product / len(pyche_list)
            theta_j /= (L - j - 1)
            theta.append(theta_j)

        # 计算SC-PseDNC-General特征向量
        feature_vector = []
...
def rna_sc_psednc_general(seq, request):
    lambda_value = int(request.form.get('sc_psednc_general_lambda_value'))
    weight = float(request.form.get('sc_psednc_general_weight'))
    dimensions = int(request.form.get('sc_psednc_general_Dimensions'))
    pcplist = request.form.getlist('sc_psednc_general_param3')
    pcplist = [int(x)-1 for x in pcplist]
    df = pd.read_excel("feature_extraction/pychemdata/Table 5.xlsx", sheet_name='Sheet1')
    data = sc_psednc_general(seq, df, pcplist, lambda_value, weight)
...
if __name__ == '__main__':
    ...
    # 调用函数
    feature_vectors = sc_psednc_general(rna_sequences, pyche_df, pyche_list, lambda_value, weight)
