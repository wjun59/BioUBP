...
if __name__ == '__main__':
    sequences = ["AUUUGCACCGAUUUUGCACCG", "AUUUGCACCGAUUUUGCACCG", "AUUUGCACCGAUCUGCACCG" ]
    sequences2 = ["AUUC", "AUUC", "ACCG"]
    lag = 1
    df = pd.read_excel("../pychemdata/Table 5.xlsx", sheet_name='Sheet1')
    pcplist = [1, 2, 3, 10, 22]  # 这个是Table5中选择的理化性质(代表第一个，第二个。。。 从1开始)

    data = dac_feature_extraction(sequences2, lag, pcplist, df)
    print(data)
