...
# 计算交叉相关因子θ
def compute_theta_sc(sequence, lambda_val, standardized_values, pcplist):
    """
    计算序列的交叉相关因子 (θ)。
    Args:
        sequence (str): 蛋白质序列。
    ...
    for d in range(1, lambda_val + 1):
        ...
        for i in range(L - d):
            ...
            for idx1 in pcplist:
                for idx2 in pcplist:
                    if idx1 != idx2:  # 计算不同理化性质之间的交叉相关性
                        Hu_i_1 = standardized_values[idx1][amino_acids.index(sequence[i])]
                        Hu_j_2 = standardized_values[idx2][amino_acids.index(sequence[i + d])]
                        cross_rij_sum += (Hu_i_1 - Hu_j_2) ** 2
            correlation_sum += cross_rij_sum / (len(pcplist) * (len(pcplist) - 1))
        theta[d - 1] = correlation_sum / (L - d)

    return theta


# 提取单个序列的 SC-PseAAC-General 特征向量
def compute_sc_pseaac_general(sequence, lambda_val, weight, standardized_values, pcplist):
    """
    提取单个蛋白质序列的 SC-PseAAC-General 特征向量。
    Args:
        sequence (str): 蛋白质序列。
        lambda_val (int): 最大延迟等级。
        weight (float): 权重因子。
    ...
    # 计算 θ 值
    theta = compute_theta_sc(sequence, lambda_val, standardized_values, pcplist)
...
# 提取多个序列的特征向量
def extract_sc_pseaac_general(sequences, lambda_val, weight, pcplist, df):
    """
    提取多个蛋白质序列的 SC-PseAAC-General 特征向量。
    Args:
    ...
    for seq in sequences:
        features = compute_sc_pseaac_general(seq, lambda_val, weight, standardized_values, pcplist)
        feature_matrix.append(features)

    return np.array(feature_matrix)


def pro_SC_PseAACG(seq, request):
    lambda_val = int(request.form.get('SCPseAACGeneral_lambda'))
    weight = float(request.form.get('SCPseAACGeneral_Weight'))
    dimensions = int(request.form.get('SCPseAACGeneral_Dimensions'))
    pcplist = request.form.getlist('SCPseAACGeneral_param3')
    pcplist = [int(x) for x in pcplist]
    df = pd.read_excel("feature_extraction/pychemdata/Table 7.xlsx", sheet_name='Sheet11')
    data = extract_sc_pseaac_general(seq, lambda_val, weight, pcplist, df)
...
if __name__ == "__main__":
    ...
    # 提取特征
    features_df = extract_sc_pseaac_general(sequences, lambda_val, weight, pcplist, df)
    print(features_df)
